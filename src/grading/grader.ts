...
export class Grader {
    ...
    private detectType(filePath: string): 'aura' | 'vf' {
        if (filePath.endsWith('.cmp') || filePath.includes('/aura/')) return 'aura';
        if (filePath.endsWith('.page') || filePath.includes('/pages/')) return 'vf';
        // Default fallback - check file extension
        return filePath.endsWith('.page') ? 'vf' : 'aura';
    }
        ...
        async function walk(currentDir: string) {
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory()) {
                    ...
                    if (type !== 'aura' && entry.name.endsWith('.page')) {
                        results.push(fullPath);
                    }
                }
            }
